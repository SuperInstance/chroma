...
impl AttachedFunctionOrchestrator {
    ...
    fn require_input_batches(
        &self,
    ) -> Result<&[FunctionExecutionBatch], AttachedFunctionOrchestratorError> {
        ...
        Ok(self.input_batches())
    }

    fn single_input_collection_info(
        &self,
    ) -> Result<&CollectionCompactInfo, AttachedFunctionOrchestratorError> {
        let input_batches = self.require_input_batches()?;
        if input_batches.len() != 1 {
            return Err(AttachedFunctionOrchestratorError::InvariantViolation(
                format!(
                    "Expected exactly one input batch, found {}",
                    input_batches.len()
                ),
            ));
        }
        Ok(&input_batches[0].collection_info)
    }
    ...
    fn get_input_database_name(
        &self,
    ) -> Result<chroma_types::DatabaseName, AttachedFunctionOrchestratorError> {
        let input_batches = self.require_input_batches()?;
        let database_name = input_batches[0].collection_info.collection.database.clone();
        for batch in &input_batches[1..] {
            if batch.collection_info.collection.database != database_name {
                return Err(AttachedFunctionOrchestratorError::InvariantViolation(
                    "All input batches must share the same database".to_string(),
                ));
            }
        }
        chroma_types::DatabaseName::new(database_name)
    ...
    fn get_input_tenant(
        &self,
    ) -> Result<String, AttachedFunctionOrchestratorError> {
        let input_batches = self.require_input_batches()?;
        let tenant_id = input_batches[0].collection_info.collection.tenant.clone();
        for batch in &input_batches[1..] {
            if batch.collection_info.collection.tenant != tenant_id {
                return Err(AttachedFunctionOrchestratorError::InvariantViolation(
                    "All input batches must share the same tenant".to_string(),
                ));
            }
        }
        Ok(tenant_id)
    ...
    fn shared_input_cmek(
        ...
        for batch in &input_batches[1..] {
            ...
            if cmek != seed_cmek {
                return Err(AttachedFunctionOrchestratorError::InvariantViolation(
                    "All input batches must share the same CMEK policy".to_string(),
                ));
            }
        }
        Ok(seed_cmek)
    ...
    fn response_job_id(
        &self,
    ) -> Result<JobId, AttachedFunctionOrchestratorError> {
        if self.input_batches.len() == 1 {
...
impl Orchestrator for AttachedFunctionOrchestrator {
    ...
    async fn initial_tasks(
        ...
        let collection_info = match self.single_input_collection_info() {
            Ok(info) => info,
            Err(e) => {
                self.terminate_with_result(Err(e), ctx).await;
                return vec![];
    ...
    async fn handle(
        ...
        match message.attached_function {
            Some(attached_function) => {
                ...
                let database_name = match self.get_input_database_name() {
                    Ok(name) => name,
                    Err(e) => {
                        tracing::error!(
                            "Invalid database name in input collection: {}",
                            self.require_input_batches()
                                .and_then(|batches| {
                                    batches
                                        .first()
                                        .map(|batch| batch.collection_info.collection.database.clone())
                                        .ok_or(AttachedFunctionOrchestratorError::InvariantViolation(
                                            "Attached function execution requires at least one input batch"
                                                .to_string(),
                                        ))
                                })
